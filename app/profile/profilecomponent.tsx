...
export default function CreateEditProfilePage({ isEdit }: { isEdit: boolean }) {
    ...
    const { editProfile } = useEditMatrimonialProfile()
    const { mutate: editProfileImage } = useEditProfileImage();
    const { data, isLoading } = useMatrimonialProfileByUserId(session?.user?.id || "")
    ...
    // Support both File and API image objects
    const [uploadedImages, setUploadedImages] = useState<(File | { id: string; url: string })[]>([])
    ...
    const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isEdit && formData?.id && filesToAdd.length > 0) {
            editProfileImage({ profileId: formData?.id, images: [filesToAdd[0]], userId: session?.user?.id || "" })
        }
        setUploadedImages((prev) => [...prev, ...filesToAdd])
    }

    const removeImage = (index: number, id: string) => {
        if (id) {
            deleteImage(id)
            // Optionally, handle deletion of image from server if it has an ID
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        if (isEdit) {
            ...
            // Create a shallow copy of formData without the images property
            const { images, ...dataWithoutImages } = formData

            editProfile.mutate({ id: formData?.id, data: dataWithoutImages })
        }
        else {
            e.preventDefault()
            if (!session?.user?.id) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-orange-50 to-white">
            ...
            <div className="bg-white shadow-sm border-b">
                <div className="max-w-7xl mx-auto px-4 py-4">
                    <PageHeader
                        title={isEdit ? "प्रोफाइल संपादित करें / Edit Profile" : "नई प्रोफाइल बनाएं / Create New Profile"}
                        description="अपनी मैट्रिमोनियल प्रोफाइल की जानकारी भरें / Please fill in your matrimonial profile details"
                        showBack={true}
                        backHref="/profiles"
                    // showBreadcrumb={true}
                    />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-xl text-orange-600">फोटो अपलोड करें / Upload Photo</CardTitle>
                                    <CardDescription>अपनी स्पष्ट तस्वीरें अपलोड करें (अधिकतम 3 फोटो, प्रत्येक 5MB तक)</CardDescription>
                                    <CardDescription>Upload your clear photos (Maximum 3 photos, each up to 5MB)</CardDescription>
                                        ...
                                        {uploadedImages.length < 3 && (
                                            <label className="aspect-square border-2 border-dashed border-orange-300 rounded-lg flex items-center justify-center hover:border-orange-400 cursor-pointer">
                                                ...
                                                <div className="text-center">
                                                    ...
                                                    <p className="text-sm text-orange-600">फोटो अपलोड करें / Upload Photo</p>
                                                    <p className="text-xs text-gray-500">अधिकतम 5MB / Maximum 5MB</p>
                                                </div>
                                            </label>
                                        )}
                                    </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="lastName">उपनाम / Last Name *</Label>
                                        <Input
                                            id="lastName"
                                            placeholder="उपनाम"
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="petName">पसंदीदा नाम / Preferred Name *</Label>
                                        <Input
                                            id="petName"
                                            placeholder="पसंदीदा नाम"
                                    ...
                                    {/* Other new fields: Email, Employer, Mobile */}
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="mobileNumber">मोबाइल नंबर / Mobile Number</Label>
                                            <Input
                                                id="mobileNumber"
                                                type="tel"
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="employerOrganizationName">नियोजक / Employer/Organization Name</Label>
                                        <Input
                                            id="employerOrganizationName"
                                            placeholder="कंपनी/संगठन का नाम"
                                            value={formData.employerOrganizationName || ""}
                                            onChange={(e) => handleInputChange("employerOrganizationName", e.target.value)}
                                        />
                                    </div>
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-xl text-orange-600">शिक्षा और करियर / Education and Career</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-xl text-orange-600">जीवनसाथी की प्राथमिकताएं / Life Partner Preferences</CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                            ...
                            <div className="flex justify-end">
                                <Button
                                    ...
                                    disabled={createProfile.isPending || editProfile.isPending}
                                >
                                    {editProfile.isPending
                                        ? "प्रोफाइल संपादित किया जा रहा है... (Editing...)"
                                        : createProfile.isPending
                                            ? "प्रोफाइल बनाई जा रही है... (Creating...)"
                                            : isEdit
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-8 space-y-6">
                            ...
                            <Card>
                                <CardHeader>
                                    <CardTitle className="text-lg text-orange-600">आपकी प्रोफाइल / Your User Id </CardTitle>
                                </CardHeader>
                                <CardContent className="space-y-4">
                                    <div className="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-900">{session?.user?.email || "उपयोगकर्ता"}</h3>
                                        </div>
                                    </div>
